...
-- UnitAura = DurationLib.UnitAuraWithBuffs
...
function f:UNIT_AURA(event, unit)
    ...
    for i=1,100 do
        AuraReturn = {DurationLib.UnitAuraWithBuffs(unit, i, "HELPFUL")}
        if AuraReturn[10] == nil or not DMW.Enums.DispelTypes.Magic[AuraReturn[10]] then
            break
        end
        ...
        -- print(AuraUtil.FindAuraByName(AuraReturn[1], "target"))
        if DMW.Tables.AuraCache[unitGUID] == nil then
            DMW.Tables.AuraCache[unitGUID] = {}
        end
        -- if DMW.Tables.AuraCache[Unit] == nil then
        --     DMW.Tables.AuraCache[Unit] = {}
        -- end
        if DMW.Tables.AuraCache[unitGUID][AuraReturn[1]] == nil then
            DMW.Tables.AuraCache[unitGUID][AuraReturn[1]] = {
...
function AuraCache.Event(...)
    ...
    if destGUID and (event == "SPELL_AURA_APPLIED" or event == "SPELL_AURA_APPLIED_DOSE" or event == "SPELL_AURA_REMOVED_DOSE" or event == "SPELL_AURA_REFRESH" or event == "SPELL_AURA_REMOVED" or event == "SPELL_PERIODIC_AURA_REMOVED" or
    ...
    elseif event == "PARTY_KILL" then
        if DMW.Tables.AuraCache[destGUID] then
            DMW.Tables.AuraCache[destGUID] = nil
        end
    ...
    elseif event == "SPELL_DISPELL" then
        ...
        if DMW.Tables.AuraCache[destGUID][dispelledName] then
