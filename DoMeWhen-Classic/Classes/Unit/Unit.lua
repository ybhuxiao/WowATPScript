...
function Unit:Update()
    ...
    self.Distance = self:GetRawDistance()
    self.ReachDistance = self:GetDistance()
    ...
    if self.Name == "未知目标" or self.Name == "Unknown" then
...
function Unit:HasThreat()
    ...
    elseif DMW.Enums.EnemyBlacklist[self.ObjectID] then
        return false
    elseif DMW.Player.Instance ~= "none" and (DMW.Settings.profile.HUD.Solo == 1 and UnitIsVisible("target") and UnitIsUnit(self.Pointer, "target") or UnitAffectingCombat(self.Pointer)) then
        return true
