...
function Rotation.Active()
    if DMW.Settings.profile.HUD.Rotation == 1
            and not UnitIsDeadOrGhost("player")
            and (not DMW.Helpers.Rotation.CastingCheck or not DMW.Player.Casting)
            and not (IsMounted() or IsFlying())
            and not DMW.Player.NoControl
            and (not DMW.Helpers.Rotation.StandingCheck or DMW.Player:Standing()) then
        return true
    end
    return false
end
...
function Rotation.Distance(X1, Y1, Z1, X2, Y2, Z2)
