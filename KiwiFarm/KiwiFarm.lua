...
local function FmtMoney(money)
	...
	return format( config.moneyFmt or "%d|cffffd70a金|r %d|cffc7c7cf银|r %d|cffeda55f铜|r", gold, silver, copper)
end
...
local function FmtMoneyShort(money)
    ...
    local silver = floor( (money % COPPER_PER_GOLD) / COPPER_PER_SILVER )
    local copper = floor(  money % COPPER_PER_SILVER )
	if silver>0 then str = format( "%s %d|cffc7c7cf银|r", str, silver) end
	if copper>0 then str = format( "%s %d|cffeda55f铜|r", str, copper) end
	if gold>0 or str=='' then str = format( "%d|cffffd70a金|r%s", gold, str)  end
	return strtrim(str)
end
...
local function FmtMoneyPlain(money)
	if money then
		...
		local silver = floor( (money % COPPER_PER_GOLD) / COPPER_PER_SILVER )
		local copper = floor(  money % COPPER_PER_SILVER )
		return format( "%d金 %d银 %d铜", gold, silver, copper)
	end
end
...
do
	local function fmtLoot(itemLink, quantity, money, pref )
		...
		if itemLink then
			...
			return format("%s你获得物品:%s", prefix, FmtMoneyShort(money) )
		end
	end
	local notified = {}
	local channels = {
		...
		combat = function(itemLink, quantity, money)
			if CombatText_AddMessage then
				...
				print('|cFF7FFF72KiwiFarm:|r 未启用暴雪浮动战斗文本，请转到界面选项>战斗以启用此功能.')
			end
		end,
		...
		msbt = function(itemLink, quantity, money)
			if MikSBT then
				...
				print('|cFF7FFF72KiwiFarm:|r 警告，未安装MikScrollingCombatText（MSBT）插件.')
			end
		end,
...
do
	...
	function PrepareText()
		...
		if not disabled.reset then
			text_header = text_header .. "重置次数:\n"
			text_mask   = text_mask   .. "%s%d|r||%s%02d:%02d|r\n"  -- last reset
		end
		-- count data
		if not disabled.count then
			-- mobs killed
			text_header = text_header .. "击杀数:\n"
			text_mask   = text_mask   .. "%d||%d\n"
			-- items looted
			text_header = text_header .. "获得物品数:\n"
			text_mask   = text_mask   .. "%d\n"
		end
		-- gold cash & items
		text_header = text_header .. "金币:\n物品价值:\n"
...
addon:SetScript("OnEvent", function(frame, event, name)
	...
	LibStub("LibDBIcon-1.0"):Register(addonName, LibStub("LibDataBroker-1.1"):NewDataObject(addonName, {
		...
		OnTooltipShow = function(tooltip)
			...
			tooltip:AddLine("|cFFff4040Left 左键|r 窗口显示/隐藏\n|cFFff4040右键k|r 打开设置", 0.2, 1, 0.2)
		end,
	}) , config.minimapIcon)
	-- events
...
do
	...
	local function getSessionText()
		return (session.startTime and '暂停') or (session.duration and '继续') or '开始'
	end
	...
	do
		...
		menuZones = { init = function(menu)
			if not menu[1] then
				...
				menu[#menu+1] = { text = '(+)添加当前区域', notCheckable = true, func = ZoneAdd }
			end
		end	}
	end
	...
	do
		...
		menuQualitySources = {
			{ text = '卖店价',              value = 'vendor',                     isNotRadio = true, keepShownOnClick = 1, checked = checked, func = set },
			{ text = 'Auctionator: 市场价', value = 'Atr:DBMarket', arg1 = 'Atr', isNotRadio = true, keepShownOnClick = 1, checked = checked, func = set },
			{ text = 'Auctionator: 分解价',   value = 'Atr:Destroy' , arg1 = 'Atr', isNotRadio = true, keepShownOnClick = 1, checked = checked, func = set },
			{ text = 'TSM4: 市场价',        value = 'DBMarket',     arg1 = 'TSM', isNotRadio = true, keepShownOnClick = 1, checked = checked, func = set },
			{ text = 'TSM4: 一口价',          value = 'DBMinBuyout',  arg1 = 'TSM', isNotRadio = true, keepShownOnClick = 1, checked = checked, func = set },
			{ text = 'TSM4: 分解价',          value = 'Destroy',      arg1 = 'TSM', isNotRadio = true, keepShownOnClick = 1, checked = checked, func = set },
	...
	do
		...
		local function deleteItem(itemLink, confirm)
			...
			C_Timer.After(.1,function()
				addon:ConfirmDialog( format("%s\n此道具没有定义的价格。是否要删除此道具?",itemLink), function() deleteItem(itemLink) end)
			end)
		end
		...
		local function getText(info, level)
			...
			return format( 'Price: %s', price and FmtMoneyShort(price) or '未定义')
		end
		-- submenu: item price sources
		menuItemSources = {
			{ text = getText,	  				  value = 'user',         				isNotRadio = true, keepShownOnClick = 1, checked = checked, func = set },
			{ text = '商店价',              value = 'vendor',                     isNotRadio = true, keepShownOnClick = 1, checked = checked, func = set },
			{ text = '市场价', value = 'Atr:DBMarket', arg1 = 'Atr', isNotRadio = true, keepShownOnClick = 1, checked = checked, func = set },
			{ text = '分解价',   value = 'Atr:Destroy' , arg1 = 'Atr', isNotRadio = true, keepShownOnClick = 1, checked = checked, func = set },
			{ text = 'TSM4: 市场价',        value = 'DBMarket',     arg1 = 'TSM', isNotRadio = true, keepShownOnClick = 1, checked = checked, func = set },
			{ text = 'TSM4: 一口价',          value = 'DBMinBuyout',  arg1 = 'TSM', isNotRadio = true, keepShownOnClick = 1, checked = checked, func = set },
			{ text = 'TSM4: 分解价',          value = 'Destroy',      arg1 = 'TSM', isNotRadio = true, keepShownOnClick = 1, checked = checked, func = set },
			init = InitPriceSources,
	...
	do
		local function getText(info)
			...
			return value and format("%s: %d", info.value, value ) or '无'
		end
		menuKilledMobs = {
			init = function(menu)
				defMenuStart(menu)
	...
	local menuStatsMisc = {
		{ text = '清除获得的道具数',  disabled = function() return not next(stats.lootedItems) end, notCheckable = true, func = function()
			addon:ConfirmDialog( "是否要删除此次游戏中存储的所有拾取物 ?", function()
				wipe(stats.lootedItems)
				timeLootedItems = time()
			end)
		end	},
		{ text = '清除怪物击杀数',  disabled = function() return not next(stats.killedMobs) end, notCheckable = true, func = function()
			addon:ConfirmDialog( "是否要删除此次游戏中存储的所有怪物击杀数 ?", function()
				wipe(stats.killedMobs)
			end)
		end	},
		{ text = '清除所有数据', notCheckable = true, func = function()
			local stats = stats
			addon:ConfirmDialog( "是否要删除此次游戏中存储的所有数据 ?", function()
				if stats._type then
	...
	local menuStats = {
		...
		{ notCheckable = true },
		{ notCheckable = true },
		{ text = '获得的物品数', notCheckable = true, hasArrow = true, menuList = menuGoldQuality },
		{ text = '怪物击杀数',  notCheckable = true, hasArrow = true, menuList = menuKilledMobs  },
		{ text = '维护',  notCheckable = true, hasArrow = true, menuList = menuStatsMisc   },
		init = function(menu,level)
			local curTime = time()
			...
			menu[1].text = format("刷本事件: %s", FmtDuration(ftime))
			menu[2].text = format("金币: %s", FmtMoney(stats.moneyCash))
			menu[3].text = format("道具价值: %s", FmtMoney(stats.moneyItems))
			menu[4].text = format("小时/金币: %s", FmtMoney(mhour))
			menu[5].text = format("总金币: %s", FmtMoney(money))
			menu[6].text = format("物品获得 (%d)", stats.countItems)
			menu[7].text = format("怪物击杀 (%d)", stats.countMobs)
			timeLootedItems = curTime
		end,
	}
	...
	local menuDaily = {	init = function(menu)
		...
		for i=1,7 do
			key, pre = date("%Y/%m/%d", tim), pre and date("%m/%d", tim) or '今天'
			local data = config.daily[key]
			if data then
	...
	do
		...
		menuCollect = {
			{ text = '总统计数据', notCheckable = true, isTitle = true},
			{ text = '详细怪物统计',  value = 'total', arg1 = 'killedMobs',  keepShownOnClick = 1, isNotRadio = true, checked = checked, func = set },
			{ text = '详细道具统计', value = 'total', arg1 = 'lootedItems', keepShownOnClick = 1, isNotRadio = true, checked = checked, func = set },
			{ text = '每日统计',  notCheckable = true, isTitle = true},
			{ text = '详细怪物统计 info',  value = 'daily', arg1 = 'killedMobs',  keepShownOnClick = 1, isNotRadio = true, checked = checked, func = set },
			{ text = '详细道具统计', value = 'daily', arg1 = 'lootedItems', keepShownOnClick = 1, isNotRadio = true, checked = checked, func = set },
			{ text = '区域统计',   notCheckable = true, isTitle = true},
			{ text = '详细怪物统计',  value = 'zone',  arg1 = 'killedMobs',  keepShownOnClick = 1, isNotRadio = true, checked = checked, func = set },
			{ text = '详细物品统计', value = 'zone',  arg1 = 'lootedItems', keepShownOnClick = 1, isNotRadio = true, checked = checked, func = set },
		}
	end

	-- menu: main
	local menuMain = {
		{ text = '爆本助手（小狼汉化）', notCheckable = true, isTitle = true },
		{ text = getSessionText,       notCheckable = true, func = setSession },
		{ text = '本次副本完成',     notCheckable = true, disabled = function() return not (session.startTime or session.duration) end, func = SessionFinish },
		{ text = '重置副本',    notCheckable = true, func = ResetInstances },
		{ text = '统计',         notCheckable = true, isTitle = true },
		{ text = '本次',            notCheckable = true, hasArrow = true, value = 'session', menuList = menuStats },
		{ text = '今日',              notCheckable = true, hasArrow = true, menuList = menuDaily },
		{ text = '区域',              notCheckable = true, hasArrow = true, menuList = menuZone },
		{ text = '总计',             notCheckable = true, hasArrow = true, value = 'total',   menuList = menuStats },
		{ text = '重置',             notCheckable = true, hasArrow = true, menuList = menuResets },
		{ text = '设置',           notCheckable = true, isTitle = true },
		{ text = '道具价值', notCheckable = true, hasArrow = true, menuList = {
			...
			{ text = '特殊道具', notCheckable= true, hasArrow = true, value = 'specific', menuList = menuPriceItems },
			{ text = '忽略附魔材料', isNotRadio = true, keepShownOnClick = 1,
		...
		{ text = '通知', notCheckable = true, hasArrow = true, menuList = {
			...
			{ text = '所有被拾取的道具', value = 'price', notCheckable = true, hasArrow = true, menuList = menuNotify },
			{ text = '金币拾取', value = 'money', notCheckable = true, hasArrow = true, menuList = menuNotify },
		} },
		{ text = '其他', notCheckable= true, hasArrow = true, menuList = {
			{ text = '显示信息', notCheckable= true, hasArrow = true, menuList = {
				{ text = '爆本&重置',      value = 'reset',   isNotRadio = true, keepShownOnClick = 1, checked = DisplayChecked, func = SetDisplay },
				{ text = '怪物&道具计数', value = 'count',   isNotRadio = true, keepShownOnClick = 1, checked = DisplayChecked, func = SetDisplay },
				{ text = '由品质高低显示',  value = 'quality', isNotRadio = true, keepShownOnClick = 1, checked = DisplayChecked, func = SetDisplay },
			} },
			{ text = '金币格式', notCheckable = true, hasArrow = true, menuList = {
				{ text = '999|cffffd70a金|r 99|cffc7c7cf银|r 99|cffeda55f铜|r', value = '', 							    checked = MoneyFmtChecked, func = SetMoneyFmt },
				{ text = '999|cffffd70a金|r 99|cffc7c7cf银|r', 				 value = '%d|cffffd70a金|r %d|cffc7c7cf银|r', checked = MoneyFmtChecked, func = SetMoneyFmt },
				{ text = '999|cffffd70a金|r', 								 value = '%d|cffffd70a金|r', 				checked = MoneyFmtChecked, func = SetMoneyFmt },
			} },
			{ text = '数据收集', notCheckable= true, hasArrow = true, menuList = menuCollect },
			{ text = '刷本区域',   notCheckable= true, hasArrow = true, menuList = menuZones },
		} },
		{ text = '外观', notCheckable= true, hasArrow = true, menuList = {
			{ text = '框架锚点', notCheckable= true, hasArrow = true, menuList = {
				{ text = '左上',     value = 'TOPLEFT',     checked = AnchorChecked, func = SetAnchor },
				{ text = '右上',    value = 'TOPRIGHT',    checked = AnchorChecked, func = SetAnchor },
				{ text = '左下',  value = 'BOTTOMLEFT',  checked = AnchorChecked, func = SetAnchor },
				{ text = '右下t', value = 'BOTTOMRIGHT', checked = AnchorChecked, func = SetAnchor },
				{ text = '左',   		 value = 'LEFT',   		checked = AnchorChecked, func = SetAnchor },
				{ text = '右',  		 value = 'RIGHT',  		checked = AnchorChecked, func = SetAnchor },
				{ text = '上',    		 value = 'TOP',    		checked = AnchorChecked, func = SetAnchor },
				{ text = '下', 		 value = 'BOTTOM', 		checked = AnchorChecked, func = SetAnchor },
				{ text = '中', 		 value = 'CENTER', 		checked = AnchorChecked, func = SetAnchor },
			} },
			{ text = '框架宽度', notCheckable= true, hasArrow = true, menuList = {
				{ text = '增加(+)',   value =  1,  notCheckable= true, keepShownOnClick=1, func = SetWidth },
				{ text = '减少(-)',   value = -1,  notCheckable= true, keepShownOnClick=1, func = SetWidth },
				{ text = '默认',       value =  0,  notCheckable= true, keepShownOnClick=1, func = SetWidth },
			} },
			{ text = '文字字体', notCheckable= true, hasArrow = true, menuList = menuFonts },
			{ text = '文字大小', notCheckable= true, hasArrow = true, menuList = {
				{ text = '增加(+)',  value =  1,  notCheckable= true, keepShownOnClick=1, func = SetFontSize },
				{ text = '减少(-)',  value = -1,  notCheckable= true, keepShownOnClick=1, func = SetFontSize },
				{ text = '默认 (14)', value =  0,  notCheckable= true, keepShownOnClick=1, func = SetFontSize },
			} },
			{ text ='背景颜色 ', notCheckable = true, hasColorSwatch = true, hasOpacity = true,
				get = function() return unpack(config.backColor) end,
				set = function(info, ...) config.backColor = {...}; SetBackground(); end,
			},
			{ text = '隐藏框架', notCheckable = true, func = function() addon:Hide() end },
		} },
	}
